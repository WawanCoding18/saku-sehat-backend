...
export const streamGroq = async (message: string, res: any): Promise<void> => {
  ...
  const systemPromptLength = FULL_SYSTEM_INSTRUCTION_FINANCIAL_HEALTH?.length || 0;
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: FULL_SYSTEM_INSTRUCTION_FINANCIAL_HEALTH,
        },
        {
          role: "user",
          content: `Data Finansial Pengguna:\n"""${message}"""`,
        },
      ],
