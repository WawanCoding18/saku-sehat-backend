...
// ✅ Disesuaikan dengan skema ITargetTabung yang sebenarnya:
// targetNominal, terkumpulNominal, deadlineTarget, status, createdAt (dari timestamps)
const hitungTargetNabung = async (userId: string) => {
  ...
  for (const target of listTarget) {
    ...
    // Status "Aktif": hitung berdasarkan progres waktu vs progres nominal
    // createdAt dipakai sebagai proksi tanggal mulai, karena skema tidak
    // punya field tanggal mulai eksplisit
    ...
    const rasioPencapaian =
      progresDiharapkan > 0
        ...
        : 1; // baru dibuat, belum ada ekspektasi waktu, anggap on-track dulu
